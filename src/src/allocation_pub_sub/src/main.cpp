...
//城市参数初始化
std::vector<int> subcity_x;
std::vector<int> subcity_y;
std::vector<int> subcity_p0;
std::vector<int> subcity_p1;
std::vector<int> subcity_s;
std::vector<int> subcity_ID;
//小车参数初始化
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "car_qxxxxx:"<<std::endl;
    for (size_t i = 0; i < car_q.size(); ++i) {
        std::cout << car_q[i]<<std::endl;}
    //----------------------------------------------------------------第一次启动协同任务调度计算节点结束
    //数据加载成功，满足任务调度算法输入，开始执行任务调度，记录此时系统的实时时间
    auto start = std::chrono::steady_clock::now();

    // 创建 分区 对象并运行算法
    DBSCAN subregion(city_x, city_y,city_p0, city_p1,city_s, eps, minPts);
    subregion.run();
    subregion.calculateClusterStats();
    subregion.getClusterStats(subcity_x,subcity_y,subcity_p0,subcity_p1,subcity_s,subcity_ID);
    subregion.printClusters();
    city_ID = subregion.getClusterIDs();
    // 任务子分区
    CityParameters cityParams = {subcity_x,subcity_y,subcity_p0,subcity_p1,subcity_s};
    // 小车参数初始化
    CarParameters carParams = {car_x,car_y,car_p,car_q};
    // 创建联盟分配对象并执行分配，输入为第一层任务调度分区结果和无人车节点
    Assignment assignment(cityParams, carParams);
    assignment.distributePoints(); // 执行分配
    assignment.printResults();     // 输出分配结果
    car_ID = assignment.getPointRegionIDs();
    for (const int& value : car_ID) {
        std::cout <<"car_ID"<< value << std::endl;
    }
    for (const auto&ID:city_ID)
    {
        std::cout<<ID<<endl;
    }
    // 创建 Compute 对象并执行分配
    Compute computeid(city_x, city_y, city_p0, city_p1, city_s, city_ID, car_x, car_y, car_p, car_q, car_ID);
    std::vector<int> assignments = computeid.computeAssignments();
    // 输出结果
    for (size_t i = 0; i < assignments.size(); ++i) {
        std::cout << "Car " << i << " assigned to city " << assignments[i] << std::endl;
    }
	//调用算法对成本矩阵进行求解实现无人车的最优任务分配
    task_point = assignments;
    nh.setParam("task_point",task_point);
    //-------------------------------------------------------输出协同任务调度的响应时间
    auto end = std::chrono::steady_clock::now();
	std::chrono::duration<double, std::micro> elapsed = end - start; // std::micro 
    for(size_t i = 0;i<task_point.size();++i)
    {
        std::cout<<"<------无人车"<<i+1<<"--->"<<"任务"<<task_point[i]+1<<"------>"<<endl;
    }
