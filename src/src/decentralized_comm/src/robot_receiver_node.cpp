...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "robot_receiver_node");
    ros::NodeHandle private_nh("~");
    ros::NodeHandle nh;

    // 参数读取
    private_nh.param<std::string>("bind_ip", bind_ip, "192.168.1.200");
    private_nh.param<int>("bind_port", bind_port, 6666);
    ...
    while (ros::ok()) {
        ...
        if (status.ParseFromArray(request.data(), request.size())) {
            ...
            ROS_INFO("recv RobotStatus: id=%d pos=(%.2f, %.2f, %.2f), energy=%d",
                     status.robot_id(), status.x(), status.y(), status.z(), status.energy());
            car_q_real[status.robot_id()]=status.energy();
            nh.setParam("car_q_real",car_q_real);
            std::cout<<"car_q_real:"<<car_q_real[status.robot_id()]<<std::endl;
