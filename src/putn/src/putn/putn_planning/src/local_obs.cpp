...
void rcvVelodyneCallBack(const sensor_msgs::PointCloud2& velodyne_points)
{
  ...
  for (const auto& pt : (*cloud_after_PassThrough).points)
  {
      ...
      if (local_world.isFree(obstacle))
      {
        ...
        //   // 膨胀逻辑开始
        //   double inflation_radius = 0.3;  // 膨胀半径 (单位: 米)
        //   int inflation_steps = std::ceil(inflation_radius / resolution);  // 根据分辨率计算膨胀步数
        ...
        //           Vector3d inflated_obstacle(
        //               obstacle(0) + dx * resolution,
        //               obstacle(1) + dy * resolution,
        //               obstacle(2)
        //           );

        //          // 确保膨胀点在地图范围内
        //           if (inflated_obstacle(0) >= local_x_l && inflated_obstacle(0) <= local_x_u &&
        //               inflated_obstacle(1) >= local_y_l && inflated_obstacle(1) <= local_y_u)
        //          {
        //               local_world.setObs(inflated_obstacle);

        //               Vector3d inflated_obstacle_round = local_world.coordRounding(inflated_obstacle);
        //               pcl::PointXYZ inflated_pt_add;
        //               inflated_pt_add.x = inflated_obstacle_round(0);
        //               inflated_pt_add.y = inflated_obstacle_round(1);
        //               inflated_pt_add.z = inflated_obstacle_round(2);
        //               cloud_filt->points.push_back(inflated_pt_add);
        //           }
        //       }
        //   }
        // // 膨胀逻辑结束
